export class Dnd5eCommunicator {
    ...
    static STYLES = [
        { value: 'green', i18nKey: 'DNDCOM.Settings.MSGStyleGr' },
        { value: 'blue', i18nKey: 'DNDCOM.Settings.MSGStyleBl' },
        { value: 'yellow', i18nKey: 'DNDCOM.Settings.MSGStyleYe' },
        { value: 'red', i18nKey: 'DNDCOM.Settings.MSGStyleRe' },
        { value: 'damaged', i18nKey: 'DNDCOM.Settings.MSGStyleDm' },
        { value: 'undertale', i18nKey: 'DNDCOM.Settings.MSGStyleUn' }
    ];
    ...
    static debug(message, ...args) {
        const debugMode = game.settings?.get('dnd5e-communicator', 'debugMode') ?? false;
        if (debugMode) {
    ...
    static initSocketListeners() {
        ...
        game.socket.on('module.dnd5e-communicator', (payload) => {
    ...
    static _cacheSettings() {
        this.settings.globalTypingSpeed = game.settings.get('dnd5e-communicator', 'globalTypingSpeed') ?? 130;
        this.settings.voiceVolume = game.settings.get('dnd5e-communicator', 'voiceVolume') ?? 0.3;
        this.settings.fontFamily = game.settings.get('dnd5e-communicator', 'fontFamily') || 'MOSCOW2024';
        this.settings.enableTextShake = game.settings.get('dnd5e-communicator', 'enableTextShake') ?? true;
        this.settings.globalMessageWidth = game.settings.get('dnd5e-communicator', 'globalMessageWidth') ?? 40;
        this.settings.debugMode = game.settings.get('dnd5e-communicator', 'debugMode') ?? false;
    ...
    static _extractFormData(form) {
        ...
        return {
            ...
            postToChat: form.querySelector('#post-to-chat')?.checked ?? game.settings.get('dnd5e-communicator', 'postToChat'),
            postImageToChat: form.querySelector('#post-image-to-chat')?.checked ?? game.settings.get('dnd5e-communicator', 'postImageToChat')
        };
    }
    ...
    static _validateFormData(data, requireMessage = true) {
        if (!data.characterName.trim()) {
            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.NoCharacterName'));
            return false;
        }
        ...
        if (requireMessage && !data.message.trim() && !data.imagePath) {
            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.NoMessage'));
            return false;
        }
        return true;
    }
                ...
                new FilePicker({
                    ...
                    callback: async (path) => {
                        if (await this._validateFile(path)) {
                            ...
                            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.FileNotFound'));
                        }
                    }
                }).browse();
            });
     ...
     * @param {Object} [options] - Дополнительные параметры вызова
     * @param {Actor} [options.actor] - Актёр dnd5e, из которого нужно подставить имя/портрет
     *   (например, при вызове с кнопки на листе персонажа). Имеет приоритет над
     *   выбранным на сцене токеном и над последними сохранёнными значениями.
     */
    static async openCommunicatorSettings(options = {}) {
        this._cacheSettings();

        const sourceActor = options.actor ?? null;
        const selectedToken = canvas.tokens.controlled[0];
        let lastPortrait = game.settings.get('dnd5e-communicator', 'lastPortrait');
        let lastCharacterName = game.settings.get('dnd5e-communicator', 'lastCharacterName');
        const lastSound = game.settings.get('dnd5e-communicator', 'lastSound');
        const lastVoiceover = game.settings.get('dnd5e-communicator', 'lastVoiceover');
        const lastImage = game.settings.get('dnd5e-communicator', 'lastImage');
        const lastStyle = game.settings.get('dnd5e-communicator', 'lastMessageStyle');
        const fontSize = game.settings.get('dnd5e-communicator', 'messageFontSize');
        const lastTypingSpeed = game.settings.get('dnd5e-communicator', 'lastTypingSpeed');
        const globalTypingSpeed = this.settings.globalTypingSpeed;
        const lastMessageWidth = game.settings.get('dnd5e-communicator', 'lastMessageWidth');
        const globalMessageWidth = this.settings.globalMessageWidth;

        if (sourceActor) {
            // Вызов с листа dnd5e-актёра — берём реальные имя и портрет актёра
            lastCharacterName = sourceActor.name;
            lastPortrait = sourceActor.img || lastPortrait;
        ...
        // Растягиваем диалог под реальный размер экрана, чтобы все поля
        // помещались без внутреннего скролла (см. компактную 3-колоночную
        // строку Font Family/Font Size/Message Width ниже и уплотнённые
        // отступы в CSS).
        const dialogWidth = Math.max(900, Math.min(1100, Math.round(window.innerWidth * 0.94)));
        const dialogHeight = Math.max(620, Math.min(920, Math.round(window.innerHeight * 0.92)));

        new Dialog({
            title: game.i18n.localize('DNDCOM.Settings.CommunicatorSettings'),
            content: `
                <form class="dnd5e-communicator-dialog grid-2">
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.CharacterName')}</label>
                        <input type="text" id="character-name" value="${escapedCharName}" placeholder="${game.i18n.localize('DNDCOM.Settings.CharacterName')}">
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.Portrait')}</label>
                        <div class="lcm-input-group">
                            <input type="text" id="portrait-path" value="${escapedPortrait}" placeholder="${game.i18n.localize('DNDCOM.Settings.EnterPortraitURL')}">
                            <button type="button" id="select-portrait">${game.i18n.localize('DNDCOM.Settings.SelectFromLibrary')}</button>
                        </div>
                        <small class="lcm-hint">${game.i18n.localize('DNDCOM.Settings.PortraitHint')}</small>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.MessageText')}</label>
                        <textarea id="message-input" rows="3" placeholder="${game.i18n.localize('DNDCOM.Settings.MessageText')}"></textarea>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.SoundSelect')}</label>
                        <div class="lcm-input-group">
                            <input type="text" id="sound-path" value="${this._escapeHtml(lastSound || '')}" readonly placeholder="${game.i18n.localize('DNDCOM.Settings.SelectSound')}">
                            <button type="button" id="select-sound">${game.i18n.localize('DNDCOM.Settings.SelectSound')}</button>
                            <button type="button" id="clear-sound">${game.i18n.localize('DNDCOM.Settings.ClearSound')}</button>
                        </div>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.VoiceoverSelect')}</label>
                        <div class="lcm-input-group">
                            <input type="text" id="voiceover-path" value="${this._escapeHtml(lastVoiceover || '')}" readonly placeholder="${game.i18n.localize('DNDCOM.Settings.SelectVoiceover')}">
                            <button type="button" id="select-voiceover">${game.i18n.localize('DNDCOM.Settings.SelectVoiceover')}</button>
                            <button type="button" id="clear-voiceover">${game.i18n.localize('DNDCOM.Settings.ClearVoiceover')}</button>
                        </div>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.ImageSelect')}</label>
                        <div class="lcm-input-group">
                            <input type="text" id="image-path" value="${this._escapeHtml(lastImage || '')}" readonly placeholder="${game.i18n.localize('DNDCOM.Settings.SelectImage')}">
                            <button type="button" id="select-image">${game.i18n.localize('DNDCOM.Settings.SelectImage')}</button>
                            <button type="button" id="clear-image">${game.i18n.localize('DNDCOM.Settings.ClearImage')}</button>
                        </div>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.typingSpeed')} <small style="color:#999;">(${game.i18n.localize('DNDCOM.Settings.globalTypingSpeed')}: ${globalTypingSpeed})</small></label>
                        <div class="lcm-input-group" style="align-items:center;">
                            <input type="checkbox" id="use-global-speed" ${useGlobalSpeed ? 'checked' : ''}>
                            <label for="use-global-speed" style="font-weight:normal;font-size:12px;margin-bottom:0;">${game.i18n.localize('DNDCOM.Settings.useGlobalSpeed')}</label>
                        </div>
                        <div class="lcm-input-group" id="typing-speed-row" style="margin-top:5px;${useGlobalSpeed ? 'opacity:0.5;pointer-events:none;' : ''}">
                            ...
                            <button type="button" id="auto-speed-btn" style="margin-left:5px;padding:2px 8px;font-size:11px;" title="${game.i18n.localize('DNDCOM.Settings.autoSpeedHint')}">${game.i18n.localize('DNDCOM.Settings.autoSpeed')}</button>
                        </div>
                        <small class="lcm-hint">${game.i18n.localize('DNDCOM.Settings.typingSpeedHint')}</small>
                    </div>
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.MessageStyle')}</label>
                        <select id="message-style">${styleOptions}</select>
                    </div>
                    <div class="lcm-row-3">
                        <div class="lcm-form-group">
                            <label>${game.i18n.localize('DNDCOM.Settings.FontFamily')}</label>
                            <select id="font-family">${fontOptions}</select>
                        </div>
                        <div class="lcm-form-group">
                            <label>${game.i18n.localize('DNDCOM.Settings.FontSize')}</label>
                            <div class="lcm-input-group">
                        ...
                        <div class="lcm-form-group">
                            <label>${game.i18n.localize('DNDCOM.Settings.MessageWidth')}</label>
                            <div class="lcm-input-group">
                    ...
                    <div class="lcm-form-group lcm-post-to-chat-row">
                        <label class="lcm-toggle-label">
                            <input type="checkbox" id="post-to-chat" ${game.settings.get('dnd5e-communicator', 'postToChat') ? 'checked' : ''}>
                            <span>${game.i18n.localize('DNDCOM.Settings.PostToChat')}</span>
                        </label>
                        <small class="lcm-hint">${game.i18n.localize('DNDCOM.Settings.PostToChatHint')}</small>
                    </div>
                    <div class="lcm-form-group lcm-post-to-chat-row">
                        <label class="lcm-toggle-label">
                            <input type="checkbox" id="post-image-to-chat" ${game.settings.get('dnd5e-communicator', 'postImageToChat') ? 'checked' : ''}>
                            <span>${game.i18n.localize('DNDCOM.Settings.PostImageToChat')}</span>
                        </label>
                        <small class="lcm-hint">${game.i18n.localize('DNDCOM.Settings.PostImageToChatHint')}</small>
                    </div>
                </form>
                <div id="style-preview" class="lcm-form-group" style="margin-top: 8px;"></div>
            `,
            buttons: {
                send: {
                    icon: '',
                    label: game.i18n.localize('DNDCOM.Settings.Send'),
                    callback: (html) => {
                ...
                macro: {
                    icon: '',
                    label: game.i18n.localize('DNDCOM.Settings.CreateMacro'),
                    callback: async (html) => {
                        const data = this._extractFormData(html[0].querySelector('form'));
                        if (!this._validateFormData(data)) return false;

                        if (this._isMacroNameTaken(data.characterName)) {
                            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.MacroAlreadyExists'));
                            return false;
                        }
                ...
                quickMacro: {
                    icon: '',
                    label: game.i18n.localize('DNDCOM.Settings.CreateQuickMacro'),
                    callback: async (html) => {
                        const data = this._extractFormData(html[0].querySelector('form'));
                ...
                cancel: {
                    icon: '',
                    label: game.i18n.localize('DNDCOM.Settings.Cancel')
                }
            },
            default: 'send',
            render: (html) => this._renderDialogHandlers(html),
            close: (html) => this._closeDialogHandler(html)
        }, { width: dialogWidth, height: dialogHeight, classes: ['dialog', 'dnd5e-communicator-dialog'] }).render(true);
    }
    ...
    static _renderDialogHandlers(html) {
        ...
        autoSpeedBtn.addEventListener('click', async () => {
            ...
            if (!voiceoverPath) {
                ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.NoVoiceoverForAuto'));
                return;
            }

            if (!messageText || messageText.trim().length === 0) {
                ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.NoMessageForAuto'));
                return;
            }
            ...
            try {
                ...
                if (duration < 3) {
                    ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.VoiceoverTooShort'));
                    autoSpeedBtn.disabled = false;
                    autoSpeedBtn.textContent = game.i18n.localize('DNDCOM.Settings.autoSpeed');
                    return;
                }
                ...
                ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.AutoSpeedFailed'));
            } finally {
                autoSpeedBtn.disabled = false;
                autoSpeedBtn.textContent = game.i18n.localize('DNDCOM.Settings.autoSpeed');
            }
        ...
        function updatePreview() {
            ...
            previewContent.innerHTML = `
                ...
                <div style="flex-grow:1;font-family:var(--message-font);font-size:var(--message-font-size);">
                    ${game.i18n.localize('DNDCOM.Settings.MessageStyle')} - ${styleSelect.options[styleSelect.selectedIndex].text}
                </div>
            `;
    ...
    static _saveDialogSettings(data) {
        game.settings.set('dnd5e-communicator', 'lastCharacterName', data.characterName);
        game.settings.set('dnd5e-communicator', 'lastPortrait', data.portraitPath);
        game.settings.set('dnd5e-communicator', 'lastSound', data.soundPath);
        game.settings.set('dnd5e-communicator', 'lastImage', data.imagePath);
        game.settings.set('dnd5e-communicator', 'lastMessageStyle', data.style);
        game.settings.set('dnd5e-communicator', 'fontFamily', data.fontFamily);
        game.settings.set('dnd5e-communicator', 'lastTypingSpeed', data.typingSpeed);
        game.settings.set('dnd5e-communicator', 'lastMessageWidth', data.messageWidth);
    }
    ...
    static _closeDialogHandler(html) {
        ...
        if (fontSize >= 10 && fontSize <= 32) {
            game.settings.set('dnd5e-communicator', 'messageFontSize', fontSize)
                .catch(err => console.error('D&D 5e Communicator | Error saving font size:', err));
        }

        const messageWidth = Number(formElement.querySelector('#message-width-input')?.value || 40);
        if (messageWidth >= 20 && messageWidth <= 90) {
            game.settings.set('dnd5e-communicator', 'lastMessageWidth', messageWidth)
                .catch(err => console.error('D&D 5e Communicator | Error saving message width:', err));
        }

        const voiceoverPath = this._getFormValue(formElement, '#voiceover-path');
        game.settings.set('dnd5e-communicator', 'lastVoiceover', voiceoverPath);

        const postImageToChat = formElement.querySelector('#post-image-to-chat')?.checked;
        if (postImageToChat !== undefined) game.settings.set('dnd5e-communicator', 'postImageToChat', postImageToChat);
    }
    ...
    static sendCommunicatorMessage(characterName, portraitPath, message, soundPath = '', voiceoverPath = '', imagePath = '', style = 'green', fontSize = 14, fontFamily = null, typingSpeed = null, messageWidth = null, postToChat = null, postImageToChat = null) {
        ...
        game.socket.emit('module.dnd5e-communicator', {
            type: 'showMessage',
            data: messageData
        });
    ...
    static _applyTextShake(text) {
        const enableTextShake = game.settings.get('dnd5e-communicator', 'enableTextShake') ?? true;
        if (!enableTextShake) {
    ...
    static async _postToChat(data, postToChatOverride = null) {
        const enabled = postToChatOverride !== null ? postToChatOverride : game.settings.get('dnd5e-communicator', 'postToChat');
        if (!enabled) return;
        ...
        const imageHtml = (imagePath && (postImageToChat ?? game.settings.get('dnd5e-communicator', 'postImageToChat')))
        ...
        try {
            ...
            console.error('D&D 5e Communicator | Failed to post chat message:', error);
        }
    }
    ...
    static async showCommunicatorMessage(data) {
        ...
        container.id = 'dnd5e-communicator-message';
        ...
        if (voiceoverPath) {
            try {
                ...
                console.error('D&D 5e Communicator | Voiceover error:', error);
                voiceoverAudio = null;
            }
        }
        ...
        if (soundPath && !voiceoverPath) {
            try {
                ...
                console.error('D&D 5e Communicator | Sound preload error:', error);
                soundInstance = null;
            }
        }
    ...
    static async createCommunicatorMacro(characterName, portraitPath, message, soundPath, voiceoverPath, imagePath, style, fontSize, fontFamily = null, typingSpeed = null, messageWidth = null, postImageToChat = null) {
        if (!game.user.can('MACRO_SCRIPT')) {
            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.CreateMacroTextPerm'));
            return;
        }
        ...
        new Dialog({
            title: game.i18n.localize('DNDCOM.Settings.CreateMacroName'),
            content: `
                <form class="dnd5e-communicator-dialog">
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.MacroNameLabel')}</label>
            ...
            buttons: {
                create: {
                    icon: '',
                    label: game.i18n.localize('DNDCOM.Settings.Create'),
                    callback: (html) => {
                        const macroName = html[0].querySelector('#macro-name-input').value.trim();
                        if (!macroName) {
                            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.CreateMacroTextInt'));
                            return;
                        }
                        ...
                        const commandText = `// D&D 5e Communicator Macro
                        game.modules.get('dnd5e-communicator').api.sendCommunicatorMessage(
                        ...
                        Macro.create({
                            ...
                            ui.notifications.info(game.i18n.format('DNDCOM.Settings.Notifications.MacroCreated', { name: macroName }));
                        }).catch(error => {
                            ui.notifications.error(game.i18n.format('DNDCOM.Settings.Notifications.MacroError', { error }));
                            console.error('D&D 5e Communicator | Macro creation error:', error);
                        });
                    }
                },
                cancel: {
            ...
            default: 'create'
        }, { classes: ['dialog', 'dnd5e-communicator-dialog'] }).render(true);
    }
    ...
    static async createQuickCommunicatorMacro(characterName, portraitPath, soundPath, voiceoverPath, imagePath, style, fontSize, fontFamily = null, typingSpeed = null, messageWidth = null, postImageToChat = null) {
        ...
        const messageWidthArg = messageWidth !== null ? messageWidth : 'null';
        const postImageToChatArg = postImageToChat !== null ? postImageToChat : 'null';
        const globalTypingSpeed = game.settings.get('dnd5e-communicator', 'globalTypingSpeed');
        ...
        new Dialog({
            title: game.i18n.localize('DNDCOM.Settings.CreateQuickMacroName'),
            content: `
                <form class="dnd5e-communicator-dialog">
                    <div class="lcm-form-group">
                        <label>${game.i18n.localize('DNDCOM.Settings.MacroNameLabel')}</label>
            ...
            buttons: {
                create: {
                    ...
                    callback: (html) => {
                        ...
                        const commandText = `// D&D 5e Communicator Quick Macro
(async () => {
    const globalTypingSpeed = game.settings.get('dnd5e-communicator', 'globalTypingSpeed');
    const defaultFontSize = game.settings.get('dnd5e-communicator', 'messageFontSize') || ${fontSize};
    const defaultMessageWidth = game.settings.get('dnd5e-communicator', 'lastMessageWidth') || ${messageWidthArg !== 'null' ? messageWidthArg : 40};
                ...
                <form class="dnd5e-communicator-dialog">
                    ...
                    <div class="lcm-form-group">
                        ...
                        <div class="lcm-input-group" style="align-items:center;">
                            ...
                            <label for="quick-use-global-speed" style="font-weight:normal;font-size:12px;margin-right:10px;">\\ \${game.i18n.localize('DNDCOM.Settings.useGlobalSpeed')}</label>
                        </div>
        ...
        game.modules.get('dnd5e-communicator').api.sendCommunicatorMessage(
            ...
            ${postImageToChatArg}
                        ...
                        Macro.create({
                            ...
                            ui.notifications.info(game.i18n.format('DNDCOM.Settings.Notifications.QuickMacroCreated', { name: macroName }));
                        }).catch(error => {
                            ui.notifications.error(game.i18n.format('DNDCOM.Settings.Notifications.MacroError', { error }));
                            console.error('D&D 5e Communicator | Quick macro creation error:', error);
                        });
                    }
                },
                cancel: {
    ...
    static async _validateFile(path) {
        ...
        try {
            ...
            console.error(`D&D 5e Communicator | File validation failed for ${path}:`, error);
            return false;
        }
    }
    /**
    ...
    static openSaveMessagesDialog() {
        ...
        const allowAccess = isGM || (game.settings.get('dnd5e-communicator', 'allowPlayersAccess') ?? true);

        if (!allowAccess) {
            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.Warnings.NoAccessPermission') || "Você não tem permissão para acessar os registros.");
            return;
        }

        const allowExport = isGM || (game.settings.get('dnd5e-communicator', 'allowPlayersExport') ?? false);
        ...
        const bubbles = count === 0
            ? `<div class="lcm-chat-empty">
                   ...
                   <span>${game.i18n.localize('DNDCOM.Settings.ChatLog.Empty')}</span>
               </div>`
            : messages.map((m) => {
        ...
        const header = `
            <div class="lcm-chat-header">
                ...
                <span>${game.i18n.localize('DNDCOM.Settings.ChatLog.Title')}</span>
                <span class="lcm-chat-header-count">${count}</span>
            </div>`;

        const content = `
            <div class="dnd5e-communicator-dialog lcm-save-dialog">
                ${header}
        ...
        if (allowExport) {
            buttons.exportJson = {
                ...
                label: game.i18n.localize('DNDCOM.Settings.ChatLog.ExportJSON'),
                callback: () => this._downloadMessages('json')
            };
            buttons.exportTxt = {
                icon: '<i class="fas fa-file-alt"></i>',
                label: game.i18n.localize('DNDCOM.Settings.ChatLog.ExportTXT'),
        ...
        if (isGM) {
            buttons.clear = {
                ...
                label: game.i18n.localize('DNDCOM.Settings.ChatLog.Clear'),
                callback: () => {
                    this.savedMessages = [];
                    ui.notifications.info(game.i18n.localize('DNDCOM.Settings.ChatLog.Cleared'));
                }
            };
        }
        ...
        const dlg = new Dialog({
            title: game.i18n.localize('DNDCOM.Settings.ChatLog.Title'),
            content,
            buttons,
            default: 'close',
            render: (html) => {
    ...
    static _downloadMessages(format) {
        ...
        if (!messages || messages.length === 0) {
            ui.notifications.warn(game.i18n.localize('DNDCOM.Settings.ChatLog.Empty'));
            return;
        }
        ...
        if (format === 'json') {
            ...
            content = messages.map(m => {
                ...
                const charName = m.characterName || game.i18n.localize('DNDCOM.Unknown') || 'Unknown';
